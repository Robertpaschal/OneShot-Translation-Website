    ...
    return (
        <div className="w-full md:w-[387px] p-10 bg-white rounded-[45px] shadow-md shadow-[0px_4px_4px_0px_rgba(0,0,0,0.30)] shadow-[0px_8px_12px_6px_rgba(0,0,0,0.3)] border-b-4 border-[#13536c] border-r border-l border-t flex flex-col gap-4 overflow-hidden">
            ...
            <div className="flex items-center gap-5">
                ...
                <a href={linkedin} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-[#13536c] rounded-full flex items-center justify-center">
...
const HeaderSection = () => (
    <div className="flex flex-col items-center md:flex-row md:items-start gap-8 px-6 py-7 md:px-12 md:py-12 md:ml-20 mt-0 mb-0">
        ...
        <p className="text-[21px] text-[#13536c] w-[473px] font-bold text-center md:text-left font-['DM Sans'] capitalize w-full md:w-auto">
            Meet the skilled and experienced team <br />behind our successful
            strategies.
        </p>
    </div>
);
...
const TeamMembersSection: React.FC<TeamMembersSectionProps> = ({ teamMembers, showAll, onToggleShowAll }) => (
    <div className="flex flex-wrap justify-center gap-12 px-8 py-8 md:px-20 md:py-12 mt-0 mb-12">
        {teamMembers.slice(0, showAll ? teamMembers.length : 6).map((member, index) => (
            <TeamMemberCard
                key={index}
                name={member.name}
                role={member.role}
        ...
        <div className="w-full flex justify-center mt-8">
            <button
                ...
                className="w-[269px] h-[68px] px-[35px] py-5 bg-[#13536c] rounded-[14px] justify-center items-center gap-2.5 inline-flex transition-transform transform hover:scale-105"
            >
