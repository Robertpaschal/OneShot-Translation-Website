...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, subtitle, imageUrl, isFullWidth }) => (
    <a
        href={`/services/${title.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and').replace(/\(.*?\)/g, '').replace(/-$/, '')}-${subtitle.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and').replace(/\(.*?\)/g, '').replace(/-$/, '')}`}
        className={`flex flex-col md:flex-row items-center md:justify-between gap-6 md:gap-10 p-6 md:p-10 bg-[#5ccce4] rounded-[45px] border-t border-l border-r border-[#13536c] border-b-4 border-b-[#13536c] w-full ${isFullWidth ? "md:col-span-2" : ""
    ...
    >
        ...
        <div className="flex md:hidden flex-col items-start gap-0 text-left min-h-[40px] ">
            {/* Title */}
            <div className="text-xl font-bold text-[#13536c] drop-shadow-lg">{title}</div>
            {/* Subtitle */}
            <div className="text-xl font-bold text-[#13536c] drop-shadow-lg">{subtitle}</div>
            {/* Divider */}
    ...
    return (
        <div className="w-full min-h-screen font-['DM Sans'] flex flex-col items-center justify-center mb-20 max-w-screen-xl mx-auto lg:max-w-[1440px]">
            ...
            <div className="flex flex-col md:flex-row justify-start items-center gap-7 w-full max-w-6xl mx-auto px-4 md:px-0 mb-10 ml-23">
                ...
                <p className="text-lg md:text-xl text-[#13536c] max-w-870px font-bold text-center md:text-left capitalize">
