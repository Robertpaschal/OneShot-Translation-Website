...
const NavBar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollPos > 50) {  // Arbitrary offset for sticky effect
        setIsSticky(true);
      } else {
        setIsSticky(false);
      }
  ...
  return (
    <header className={`${styles.header} ${scrollingDown ? styles.hideNav : ''} ${isSticky ? styles.stuck : ''}`}>
      <div className={styles.logoWrapper}>
        <Link href="/" passHref legacyBehavior>
          <a className={styles.logoLink}>
            <img className={styles.logo} src="/images/navbar/logo.png" alt="Company Logo" />
          </a>
        </Link>
      </div>
      <button 
      ...
      >
        {isNavOpen ? (
          ...
          <svg 
            ...
            fill="currentColor"
          >
            <path d="M3 6h18M3 12h18m-18 6h18" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          </svg>)}
      </button>
      <nav className={`${styles.nav} ${styles.navContainer} ${isNavOpen ? styles.visible : styles.hidden}`}>
      {NAV_ITEMS.map((item) => {
